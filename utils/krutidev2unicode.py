...
def getUnicode(unk_txt):
    # Python 3 port: the original assumed a Python 2 `str` (bytes) coming in and always
    # decoded it. Called from kru2uni() with a real Python 3 str (from server.py's request
    # body) -- decoding a str is a TypeError in Python 3, not the UnicodeDecodeError this
    # function's own except clause was written to catch, so this now only decodes if it
    # genuinely received bytes, and passes a str straight through unchanged otherwise.
...
def kru2uni(kru_text):
    ...
    # Python 3 port: kru_text is already a real str at this point (Python 2's own str was
    # bytes, hence the original .encode('utf-8') here) -- encoding it now would hand the
    # caller a bytes object instead of text, which server.py's /admin/convert-krutidev then
    # can't JSON-serialize directly.
    return kru_text
